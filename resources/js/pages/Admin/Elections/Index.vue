...
function closeVoting(election) {
  if (confirm(`Tutup voting untuk ${election.name}? Tindakan ini menghentikan pemilih baru masuk.`)) {
    form.patch(`/admin/elections/${election.id}/close`)
  }
}

function publishResult(election) {
  if (confirm(`Publikasikan hasil ${election.name}? Hasil akan tampil di halaman publik.`)) {
  ...
  <div class="elections">
    ...
    <header class="elections__header">
      <div class="elections__header-left">
        ...
        <h1 class="elections__title">
          Kontrol <em>Voting.</em>
        </h1>
      ...
      >
        ...
        <div class="election-card__actions">
          ...
          >
            ...
            <span class="action-btn__label">Publikasikan Hasil</span>
            <span class="action-btn__arrow">↗</span>
          </button>

          <!-- Info untuk status draft / scheduled / published -->
          <span
            v-if="election.status === 'draft' || election.status === 'scheduled' || election.status === 'result_published'"
            class="election-card__hint"
          >
            <template v-if="election.status === 'draft'">
              Pemilihan belum dijadwalkan. Buka voting untuk memulai.
            </template>
            <template v-else-if="election.status === 'scheduled'">
              Pemilihan akan dibuka sesuai jadwal.
            </template>
            <template v-else>
              Hasil sudah dipublikasikan dan tampil di halaman publik.
            </template>
          </span>
